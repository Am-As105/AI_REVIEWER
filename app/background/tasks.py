...
from app.gitlab.client import get_merge_request_diff, format_review_comment, post_comment, post_interactive_menu, update_comment
from app.ai.analyzer import analyze_diff

logger = logging.getLogger("tasks")


def _get_or_create_project_and_mr(db, payload: dict, project_id: int, mr_iid: int):
    """
    Shared helper: ensures the Project and MergeRequest rows exist.
    Returns (project, mr) tuple.
    """
    project_attrs = payload.get("project", {})
    mr_attrs = payload.get("merge_request", {}) or payload.get("object_attributes", {})

    project_name = project_attrs.get("name", f"Project {project_id}")
    mr_title = mr_attrs.get("title", f"MR {mr_iid}")
    mr_status = mr_attrs.get("state", "opened")

    project = crud.get_or_create_project(
        db, project_gitlab_id=project_id, project_name=project_name
    )
    mr = crud.get_or_create_merge_request(
        db,
        project_id=project.project_id,
        merge_request_gitlab_iid=mr_iid,
...
async def process_merge_request_event(payload: dict) -> None:
    ...
    try:
        ...
        _get_or_create_project_and_mr(db, payload, project_gitlab_id, mr_gitlab_iid)
...
async def process_emoji_event(payload: dict) -> None:
    ...
    try:
        ...
        review_type = emoji_to_review.get(emoji_name)
        if not review_type:
        ...
        if diffs:
            ...
            project, mr = _get_or_create_project_and_mr(db, payload, project_id, mr_iid)
            ...
            # Create Findings and Link GitLab Comment
            if ai_response.findings:
                ...
                if gitlab_comment_id:
                    for cf in created_findings:
                        crud.create_gitlab_comment(db, cf.finding_id, gitlab_comment_id)
